...
namespace ArchivosSecuenciales
{
    class Secuenciales
    {
        ...
        public void ModificarDatos (ListView caja)
        {
            bool apagador = false;
            DialogBoxModificarDatos baja = new DialogBoxModificarDatos();
            if(baja.ShowDialog()== DialogResult.OK)
            {
                ...
                while (matricula != null)
                {
                    ...
                    {
                        apagador = true;
                        DialogBoxAltas modifica = new DialogBoxAltas();
                        if (modifica.ShowDialog() == DialogResult.OK)
                        {
                            sw.WriteLine(modifica.matricula);
                            sw.WriteLine(modifica.nombre);
                            sw.WriteLine(modifica.paterno);
                            sw.WriteLine(modifica.materno);
                            sw.WriteLine(modifica.especialidad);
                            sw.WriteLine(modifica.semestre);
                            sw.WriteLine(modifica.promedio);
                        }
                    }
                    matricula = sr.ReadLine();
                    nombre = sr.ReadLine();
                ...
                sr.Close();
                StreamReader sr1 = new StreamReader("...Auxiliar.txt");
                StreamWriter sw1 = new StreamWriter("...Archivos.txt", false, Encoding.ASCII);
                matricula = sr1.ReadLine();
                nombre = sr1.ReadLine();
                paterno = sr1.ReadLine();
                materno = sr1.ReadLine();
                ...
                if(apagador==false)
                {
                    MessageBox.Show("La matricula del alumno no existe en los datos de archivos", "Archivos Secuenciales",
                        MessageBoxButtons.OK, MessageBoxIcon.Stop);
                }
                 else
                {
                    MessageBox.Show("Los datos del alumno se modificaron del archivo", "Archivos Secuenciales",
